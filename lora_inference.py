...
if __name__ == "__main__":
    ...
    rank_zero_info("########## RWKV LoRA Inference ##########")

    parser = ArgumentParser()

    parser.add_argument("--load_model", default="", type=str, help="path of rwkv lora model")  # full path, with .pth
    parser.add_argument("--wandb", default="", type=str)  # wandb project name. if "" then don't use wandb
    parser.add_argument("--proj_dir", default="out", type=str)
    parser.add_argument("--run_name", default='lora_demo_run', type=str,
    ...
    parser.add_argument("--merge_lora", default=True, type=bool, help="Whether to merge LoRA weights for faster inference")
    ...
    rank_zero_info(f"LoRA inference: data={args.data_file} proj={args.proj_dir} device={args.device}")
    ...
    if args.merge_lora:
        ...
        rank_zero_info("Merged LoRA weights for faster inference")
        ...
        with torch.no_grad():
            for step in range(horizon):
                ...
                full_features = torch.zeros(
                    (1, 1, model.model.model.num_vars),
                    dtype=input_seq.dtype,
                    device=input_seq.device
                )

                # Place the predictions at the selected indices
                for i, idx in enumerate(model.model.model.select_indices):
    ...
    def plot_predictions(original_data, predictions, args):
        ...
        for i, feature_idx in enumerate(select_indices):
            ...
            plt.plot(
                pred_x,
                pred_values,
                label=f"Predicted {feature_names[i]} (LoRA)",
                color='red',
                linestyle='--',
                linewidth=2
            )
            ...
            plt.legend()
            plt.title(f"{feature_names[i]} - LoRA Autoregressive Forecast")
            ...
            save_path = os.path.join(args.proj_dir, f"lora_forecast_plot_feature_{feature_idx}.png")
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            plt.close()  # Close the figure to free memory
            print(f"Saved LoRA prediction plot for feature {feature_idx} to {save_path}")

        # Save raw predictions
        np_save_path = os.path.join(args.proj_dir, "lora_predictions.npy")
        ...
        csv_path = os.path.join(args.proj_dir, "lora_predictions.csv")
        pred_df.to_csv(csv_path, index=False)

        print(f"Saved LoRA predictions to {np_save_path} and {csv_path}")
