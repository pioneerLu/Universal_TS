...
class train_callback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        args = self.args
        should_save = (args.epoch_save > 0 and (trainer.current_epoch + 1) % args.epoch_save == 0) or \
                      (trainer.current_epoch == args.epoch_count - 1)

        if should_save:
            save_path = f"{args.proj_dir}/rwkv-{args.epoch_begin + trainer.current_epoch}.pth"
        
            try:
            
                rank_zero_info(f"Saving model weights to: {save_path}")
                ...
                if trainer.is_global_zero:
                    ...
                    torch.save(cleaned_state_dict, pure_state_dict_path)
                    rank_zero_info(f"Successfully saved PURE state_dict to: {pure_state_dict_path}")

            except Exception as e:
                rank_zero_info(f"Error during model saving process: {e}")
