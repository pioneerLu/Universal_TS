########################################################################################################
# The RWKV Language Model with LoRA support - https://github.com/BlinkDL/RWKV-LM
########################################################################################################
...
from .lora_layers import LoRAWKVModel
...
class UniversalRWKVTimeSeriesLoRA(pl.LightningModule):
    """
    LoRA版本的RWKV时间序列模型
    支持LoRA微调，冻结原始权重，只训练LoRA参数
    """
    def __init__(self, args):
        ...
        # 创建基础模型
        base_model = self._create_base_model(args)

        # 应用LoRA包装
        target_modules = getattr(args, 'lora_target_modules', ['receptance', 'key', 'value', 'output'])
        if isinstance(target_modules, str):
            target_modules = [x.strip() for x in target_modules.split(',') if x.strip()]
        lora_rank = getattr(args, 'lora_rank', 8)
        lora_alpha = getattr(args, 'lora_alpha', 16)

        self.model = LoRAWKVModel(
            base_model,
    ...
    def _create_base_model(self, args):
        """创建基础RWKV模型（不包含LoRA）"""
        class BaseModel(nn.Module):
            def __init__(self, args):
                ...
                self.sma_window = getattr(args, 'sma_window', 3)
    ...
    def configure_optimizers(self):
        ...
        if not lora_params:
            rank_zero_info("Warning: No LoRA parameters found! Using all trainable parameters.")
            lora_params = self.model.get_trainable_parameters()

        name_of_trainable_params = [f"lora_param_{i}" for i in range(len(lora_params))]
        compressed_name_of_trainable_params = compress_parameter_names(name_of_trainable_params)
        rank_zero_info(f"Name of trainable LoRA parameters in optimizers: {compressed_name_of_trainable_params}")
        rank_zero_info(f"Number of trainable LoRA parameters: {len(lora_params)}")

        # 只对LoRA参数应用weight decay
        optim_groups = [{"params": lora_params, "weight_decay": self.args.weight_decay}]
    ...
    def training_step(self, batch, batch_idx):
        ...
        predicts = self(seq_x)
        targets = seq_x
        shift_predicts = predicts[..., :-self.model.model.pre_len, :].contiguous()
        shift_targets = targets[..., self.model.model.pre_len:, self.model.model.select_indices].contiguous()
        if self.model.model.loss_type == 'mse':
            loss = F.mse_loss(shift_predicts, shift_targets)

        elif self.model.model.loss_type == 'mae':
    ...
    def _save_best_model(self, val_loss):
        ...
        new_model_path = os.path.join(self.args.proj_dir, "checkpoints", f"best-lora-{val_loss:.3f}.pth")
        ...
        rank_zero_info(f"Saved new best LoRA model to {new_model_path} with val_loss: {val_loss}")

    def _plot_predictions(self, outputs, val_loss):
        plot_dir = os.path.join(self.args.proj_dir, "visualization", f"best_plots_lora_{val_loss:.3f}")
    ...
    def _plot_scale(self, preds, targets, save_dir, prefix):
        ...
        for i, (p, t) in enumerate(zip(preds, targets)):
            ...
            plt.title(f"{prefix.capitalize()} {i+1} Prediction vs Target (LoRA)")
            plt.legend()
    ...
    def generate(self, seq, length):
        ...
        for step in range(length):
            ...
            if self.model.model.select_indices is None:
                # 默认使用从start_var_idx开始的所有变量
                start_idx = getattr(self.args, 'start_var_idx', -1)
                num_selected_vars = self.model.model.num_vars - start_idx if start_idx >= 0 else self.model.model.num_vars

                # 创建新的一步，填充预测值
                new_step = torch.zeros(B, 1, self.model.model.num_vars, device=current_seq.device, dtype=current_seq.dtype)
                ...
                new_step = torch.zeros(B, 1, self.model.model.num_vars, device=current_seq.device, dtype=current_seq.dtype)
                for i, idx in enumerate(self.model.model.select_indices):
                    new_step[:, :, idx] = last_step_pred[:, :, i:i+1]

                # 复制非预测变量的最后一个值
                for i in range(self.model.model.num_vars):
                    if i not in self.model.model.select_indices:
    ...
    def merge_lora_weights(self):
        ...
        rank_zero_info("LoRA weights merged for faster inference")
