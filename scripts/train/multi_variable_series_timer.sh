...
DATA_FILE="${DATA_FILE:-$ROOT/data/benchmark/ETTm1.csv}"
CKPT="${CKPT:-}"
PROJ_DIR="${PROJ_DIR:-out/timer}"
...
python train_timer.py \
    ...
    --enable_progress_bar True --sma_window 3 --validate_only "${VALIDATE_ONLY:-0}" --dataset_type test --num_vars 1 \
    --forecast_len 96
